...
export class TmdbService {
  ...
  async seedMissingTrailers() {
    ...
    this.logger.log(
      `Found ${moviesWithoutTrailers.length} movies without a trailer.`,
    );
    ...
    for (const movie of moviesWithoutTrailers) {
      try {
        const trailerUrl = await this.findTrailerForMovie(
          movie.title,
          movie.releaseYear,
        );
  ...
  private async findTrailerForMovie(
    ...
    if (!trailerUrl) {
      this.logger.debug(
        `No YouTube trailer found for: ${title} (TMDB ID: ${tmdbId})`,
      );
    }
