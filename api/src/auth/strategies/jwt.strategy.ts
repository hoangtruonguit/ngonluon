        ...
        super({
            ...
            secretOrKeyProvider: async (request: any, rawJwtToken: string, done: Function) => {
                try {
                    ...
                    if (parts.length !== 3) return done(new UnauthorizedException('Invalid token'), null);

                    const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'));
                    const userId: string = payload?.sub;
                    if (!userId) return done(new UnauthorizedException('Invalid token payload'), null);
                    ...
                    if (!publicKey) {
                        ...
                        if (!user || !user.publicKey) {
                            return done(new UnauthorizedException('No public key found'), null);
                        }
